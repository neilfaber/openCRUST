...
def create_visualization(img, pred_mask, boundary_coords=None, illegal_mask=None):
    ...
    axes[1, 0].set_title("Mining Area Overlay (Red) with Boundary (Green)", fontsize=14, fontweight='bold')
    axes[1, 0].axis("off")
    
    # Illegal mining areas
    if illegal_mask is not None:
        illegal_overlay = np.array(img).copy()
        illegal_overlay[illegal_mask > 0] = illegal_overlay[illegal_mask > 0] * 0.3 + np.array([255, 165, 0]) * 0.7
        axes[1, 1].imshow(illegal_overlay)
        axes[1, 1].set_title("Illegal Mining (Outside Boundary)", fontsize=14, fontweight='bold')
    else:
        axes[1, 1].imshow(img)
...
def analyze_image():
    ...
    try:
        ...
        illegal_mask, illegal_area_pixels = check_boundary_violation(pred_mask, boundary_coords)
        illegal_area = float(illegal_area_pixels * 100)  # Approximate area, ensure float
...
"""
            if stats['illegal_mining_area'] > 10000:  # > 1 hectare
                report_content += "- CRITICAL: Large-scale illegal mining detected (>1 hectare)\n"
            elif stats['illegal_mining_area'] > 5000:
                report_content += "- HIGH: Significant illegal mining detected (>0.5 hectare)\n"
            elif stats['illegal_mining_area'] > 1000:
                report_content += "- MEDIUM: Moderate illegal mining detected (>0.1 hectare)\n"
            else:
